...
const Certp5copy6= () => {
  ...
  const handleChange = (e) => {
    ...
    if (e.target.name === "courseCode") fetchCourseSuggestions(e.target.value);
  };
  ...
  const handleSearch = async () => {
    try {
      ...
      if (error.response?.status === 404) alert("❌ Student not found.");
      else alert("⚠️ Error fetching student.");
  ...
  const fetchCourseSuggestions = async (value) => {
    try {
      const response = await axios.get(`https://certificate-backend.onrender.com/api/students/courses/search?q=${value}`);
  ...
  const prepareFormData = () => {
    ...
    Object.keys(formData).forEach((key) => {
      if (key !== "filePath") form.append(key, formData[key]);
    });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const requiredFields = ["fullName", "regNo", "email", "courseCode", "trainerName", "whatsappNumber", "date", "branch", "file"];
    ...
    try {
      const checkRes = await axios.get(`https://certificate-backend.onrender.com/api/students/check-regno/${formData.regNo}`);
    ...
    const certificateNumber = generateCertificateNumber(formData.fullName, formData.regNo, formData.courseCode);
    try {
      const data = prepareFormData();
      data.append("certificateNumber", certificateNumber);
      const response = await axios.post("https://certificate-backend.onrender.com/api/students", data, {
  ...
  const confirmUpdate = async () => {
    try {
      const data = prepareFormData();
      const response = await axios.put(`https://certificate-backend.onrender.com/api/students/${formData.regNo}`, data, {
        headers: { "Content-Type": "multipart/form-data" },
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await axios.delete(`https://certificate-backend.onrender.com/api/students/${formData.regNo}`);
